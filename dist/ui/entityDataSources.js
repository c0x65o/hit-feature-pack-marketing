...
function useApiList({ url, args, extraParams, }) {
    ...
    const fetchData = useCallback(async () => {
        try {
            ...
            if (args.sortBy)
                q.set('sortBy', args.sortBy);
            if (args.sortOrder)
                q.set('sortOrder', args.sortOrder);
    ...
    useEffect(() => {
        ...
        async function fetchAll() {
            try {
                ...
                if (!cancelled)
                    setLoading({ planTypes: false, activityTypes: false, vendors: false, plans: false });
            }
        }
        fetchAll();
        return () => {
    ...
    const optionSources = useMemo(() => {
        return {
            'marketing.planTypes': {
                ...
                options: [
                    { value: '', label: '— None —' },
                    ...planTypes.map((t) => ({ value: String(t.id), label: String(t.name || t.id) })),
                ],
            },
            'marketing.activityTypes': {
                ...
                options: [
                    { value: '', label: '— None —' },
                    ...activityTypes.map((t) => ({ value: String(t.id), label: String(t.name || t.id) })),
                ],
            },
            'marketing.vendors': {
                ...
                options: [
                    { value: '', label: '— None —' },
                    ...vendors.map((t) => ({ value: String(t.id), label: String(t.name || t.id) })),
                ],
            },
            'marketing.plans': {
                ...
                options: [
                    { value: '', label: '— None —' },
                    ...plans.map((t) => ({ value: String(t.id), label: String(t.title || t.id) })),
                ],
            },
        };
    ...
    if (entityKey === 'marketing.vendor') {
        return {
            ...
            useListCustomRenderers: () => ({
                createdAt: (v) => formatDateTime(v),
                updatedAt: (v) => formatDateTime(v),
                isActive: (_, row) => (Boolean(row.isActive) ? 'Active' : 'Inactive'),
            }),
        };
    }
    // Setup entities
    if (entityKey === 'marketing.planType') {
        return {
            ...
            useListCustomRenderers: () => ({
                ...
                updatedAt: (v) => formatDateTime(v),
                isActive: (_, row) => (Boolean(row.isActive) ? 'Active' : 'Inactive'),
                isSystem: (_, row) => (Boolean(row.isSystem) ? 'System' : 'Custom'),
            }),
        };
    }
