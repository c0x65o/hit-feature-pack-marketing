...
export async function GET(request) {
    try {
        ...
        if (unassignedOnly) {
            ...
            conditions.push(eq(marketingExpenses.planId, planId));
        }
        if (typeId)
            conditions.push(eq(marketingExpenses.typeId, String(typeId)));
        if (vendorId)
            conditions.push(eq(marketingExpenses.vendorId, vendorId));
        if (fromDate)
        ...
        if (search) {
            conditions.push(or(like(marketingExpenses.notes, `%${search}%`), like(marketingPlans.title, `%${search}%`), like(marketingVendors.name, `%${search}%`), like(marketingActivityTypes.name, `%${search}%`)));
        }
        let query = db
            .select({
        ...
        const items = rows.map(({ expense, plan, type, vendor }) => ({
            ...expense,
        ...
        let totalsQuery = db
            .select({ totalAmount: sql `coalesce(sum(${marketingExpenses.amount}), 0)` })
            .from(marketingExpenses)
            .leftJoin(marketingPlans, eq(marketingExpenses.planId, marketingPlans.id))
            .leftJoin(marketingActivityTypes, eq(marketingExpenses.typeId, marketingActivityTypes.id))
            .leftJoin(marketingVendors, eq(marketingExpenses.vendorId, marketingVendors.id));
...
export async function POST(request) {
    try {
        ...
        if (vendorId) {
            const [vendor] = await db.select({ id: marketingVendors.id }).from(marketingVendors).where(eq(marketingVendors.id, vendorId)).limit(1);
            if (!vendor)
