...
function isOwnedByUser(campaign, userId) {
    if (!userId)
        return false;
...
export async function PUT(request, { params }) {
    try {
        ...
        if (body?.goals !== undefined)
            updateData.goals = body.goals || null;
        if (body?.campaignTypeId !== undefined)
            updateData.campaignTypeId = body.campaignTypeId ? String(body.campaignTypeId) : null;
        if (body?.status !== undefined)
            updateData.status = parsedStatus ?? 'planned';
        if (body?.startDate !== undefined)
            updateData.startDate = startDateValue;
        if (body?.endDate !== undefined)
            updateData.endDate = endDateValue;
        if (budgetValue !== undefined)
            updateData.budgetAmount = budgetValue;
        if (body?.divisionId !== undefined)
            updateData.divisionId = normalizeScopeValue(body.divisionId);
        if (body?.departmentId !== undefined)
            updateData.departmentId = normalizeScopeValue(body.departmentId);
        if (body?.locationId !== undefined)
            updateData.locationId = normalizeScopeValue(body.locationId);
