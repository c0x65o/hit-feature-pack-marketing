...
export async function GET(request) {
    try {
        ...
        const conditions = [];
        if (activeOnly)
            conditions.push(eq(marketingPlanTypes.isActive, true));
        if (search) {
        ...
        const countQuery = db.select({ count: sql `count(*)` }).from(marketingPlanTypes);
...
export async function POST(request) {
    try {
        ...
        if (existing)
            return NextResponse.json({ error: 'A plan type with this key already exists' }, { status: 409 });
        const now = new Date();
