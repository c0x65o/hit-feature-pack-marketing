...
const campaignStatusSet = new Set(CAMPAIGN_STATUSES);
function parseCampaignStatus(value) {
    if (value === undefined || value === null || value === '')
        return null;
    const status = String(value);
    return campaignStatusSet.has(status) ? status : null;
}
...
export async function GET(request) {
    try {
        ...
        const conditions = [];
        if (statusParam)
            conditions.push(eq(marketingCampaigns.status, statusParam));
        if (campaignTypeId)
            conditions.push(eq(marketingCampaigns.campaignTypeId, String(campaignTypeId)));
        if (search) {
            conditions.push(or(like(marketingCampaigns.name, `%${search}%`), like(marketingCampaigns.description, `%${search}%`), like(marketingCampaigns.goals, `%${search}%`)));
        }
        ...
        if (mode === 'own') {
            ...
            conditions.push(or(eq(marketingCampaigns.ownerUserId, ownerKey), eq(marketingCampaigns.createdByUserId, ownerKey)));
        }
        ...
        const countQuery = db.select({ count: sql `count(*)` }).from(marketingCampaigns);
        const [countRow] = conditions.length > 0 ? await countQuery.where(and(...conditions)) : await countQuery;
        const total = Number(countRow?.count || 0);
        const rows = await query;
        const items = rows.map(({ campaign, type }) => ({
            ...campaign,
...
export async function POST(request) {
    try {
        ...
        const { name, description, goals, campaignTypeId, status, startDate, endDate, budgetAmount, ownerUserId, divisionId, departmentId, locationId, } = body || {};
        if (!name?.trim()) {
        ...
        if (campaignTypeId) {
            const [type] = await db
                ...
                .limit(1);
            if (!type)
                return NextResponse.json({ error: 'Campaign type not found' }, { status: 404 });
        }
