...
function linkingDisabled() {
    return NextResponse.json({ error: 'Project linking is disabled for marketing. Enable option enable_project_linking to use links.' }, { status: 403 });
}
export async function GET(request) {
    try {
        const opts = getMarketingOptionsFromRequest(request);
        if (!opts.enable_project_linking)
            return linkingDisabled();
        const db = getDb();
        ...
        const conditions = [];
        if (marketingEntityType)
            conditions.push(eq(marketingEntityLinks.marketingEntityType, marketingEntityType));
        if (marketingEntityId)
            conditions.push(eq(marketingEntityLinks.marketingEntityId, marketingEntityId));
        if (linkedEntityKind)
            conditions.push(eq(marketingEntityLinks.linkedEntityKind, linkedEntityKind));
        if (linkedEntityId)
            conditions.push(eq(marketingEntityLinks.linkedEntityId, linkedEntityId));
...
export async function POST(request) {
    try {
        ...
        if (!marketingEntityType)
            return NextResponse.json({ error: 'marketingEntityType is required' }, { status: 400 });
        if (!marketingEntityId)
            return NextResponse.json({ error: 'marketingEntityId is required' }, { status: 400 });
        if (!linkedEntityKind)
            return NextResponse.json({ error: 'linkedEntityKind is required' }, { status: 400 });
        if (!linkedEntityId)
            return NextResponse.json({ error: 'linkedEntityId is required' }, { status: 400 });
        const now = new Date();
...
export async function DELETE(request) {
    try {
        ...
        if (id) {
            await db.delete(marketingEntityLinks).where(eq(marketingEntityLinks.id, id));
        ...
        await db
            ...
            .where(and(eq(marketingEntityLinks.marketingEntityType, marketingEntityType), eq(marketingEntityLinks.marketingEntityId, marketingEntityId), eq(marketingEntityLinks.linkedEntityKind, linkedEntityKind), eq(marketingEntityLinks.linkedEntityId, linkedEntityId)));
        return NextResponse.json({ success: true });
    }
    catch (error) {
