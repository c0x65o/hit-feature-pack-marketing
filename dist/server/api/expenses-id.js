...
export async function GET(_request, { params }) {
    try {
        ...
        const [expense] = await db.select().from(marketingExpenses).where(eq(marketingExpenses.id, id)).limit(1);
        if (!expense)
...
export async function PUT(request, { params }) {
    try {
        ...
        const [existing] = await db.select({ id: marketingExpenses.id }).from(marketingExpenses).where(eq(marketingExpenses.id, id)).limit(1);
        if (!existing)
            return NextResponse.json({ error: 'Expense not found' }, { status: 404 });
        if (body.planId) {
        ...
        const [updated] = await db.select().from(marketingExpenses).where(eq(marketingExpenses.id, id)).limit(1);
