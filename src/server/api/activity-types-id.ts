...
export async function GET(_request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (!item) return NextResponse.json({ error: 'Activity type not found' }, { status: 404 });
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (!existing) return NextResponse.json({ error: 'Activity type not found' }, { status: 404 });
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const [existing] = await db.select({ id: marketingActivityTypes.id, isSystem: marketingActivityTypes.isSystem }).from(marketingActivityTypes).where(eq(marketingActivityTypes.id, id)).limit(1);
    if (!existing) return NextResponse.json({ error: 'Activity type not found' }, { status: 404 });
    if (existing.isSystem) return NextResponse.json({ error: 'Cannot delete system activity types' }, { status: 403 });
