...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const expenses = expensesRows.map(({ expense, type, vendor }: any) => ({
      ...expense,
    ...
    const [spendRow] = await db
      ...
      .from(marketingExpenses)
      .where(eq(marketingExpenses.planId, id as any));
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (body.allocateByType !== undefined) updateData.allocateByType = Boolean(body.allocateByType);
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    await db.update(marketingPlans).set({ isArchived: true, updatedAt: new Date() }).where(eq(marketingPlans.id, id as any));
