...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    await db.update(marketingVendors).set(updateData).where(eq(marketingVendors.id, id as any));
    const [updated] = await db.select().from(marketingVendors).where(eq(marketingVendors.id, id as any)).limit(1);
