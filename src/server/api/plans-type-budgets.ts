...
export async function GET(_request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const rows = await db
      ...
      .where(eq(marketingPlanTypeBudgets.planId, planId as any));

    const items = await Promise.all(
      rows.map(async ({ budget, type }: any) => {
        const [spendRow] = await db
          .select({ actualAmount: sql<number>`coalesce(sum(${marketingExpenses.amount}), 0)` })
          .from(marketingExpenses)
          .where(and(eq(marketingExpenses.planId, planId as any), eq(marketingExpenses.typeId, budget.activityTypeId)));
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (!entries) return NextResponse.json({ error: 'typeBudgets must be an array' }, { status: 400 });
    ...
    for (const entry of entries) {
      ...
      if (!Number.isFinite(plannedAmount) || plannedAmount < 0) continue;
      ...
      const [existing] = await db
        ...
        .from(marketingPlanTypeBudgets)
        .where(and(eq(marketingPlanTypeBudgets.planId, planId as any), eq(marketingPlanTypeBudgets.activityTypeId, String(activityTypeId))))
        .limit(1);
      ...
      if (existing) {
        const [updated] = await db
          ...
          .where(eq(marketingPlanTypeBudgets.id, existing.id as any))
          .returning();
